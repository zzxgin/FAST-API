...
|------|---------|------|----------|------|--------|----------|
| 阶段一 | F1-T1~F1-T5 | 目录结构/环境/数据库 | docs/README.md、docker/docker-compose.yml、scripts/init_db.sql、app/models/、app/schemas/ 目录等文件齐全 | ☐ Pending<br>☐ Done<br>☐ Tested | 后端负责人 | |
| 阶段二 | F2-T1~F2-T5 | 用户/任务/审核/奖励/工具模块 | app/api/、app/models/、app/schemas/、app/crud/、app/core/ 相关代码（含 reward.py），API 跑通，权限与状态流转无误 | ☐ Pending<br>☐ Done<br>☐ Tested | 后端负责人 | |
| 阶段三 | F3-T1~F3-T5 | 业务 API/集成 | app/api/、app/schemas/、app/crud/、app/core/ 主要业务 API 可用（含 reward 相关），集成测试通过 | ☐ Pending<br>☐ Done<br>☐ Tested | 后端负责人 | |
| 阶段四 | F4-T1~F4-T4 | 测试/文档/运维 | tests/、docs/openapi.json、docker/、.github/workflows/ 等文件齐全，测试覆盖、文档与部署脚本齐全 | ☐ Pending<br>☐ Done<br>☐ Tested | 后端负责人 | |
